...
protected:
	...
	//implementation
public:
	void CreateInterruptVectorTable(); // make a table of jumps at 0x800000
	void MakeDissCode();//Automatically disassembles the code and tries to make subroutines
	//void MakeSubroutines(void);//No longer used
	void MakeSegments();//Makes the segments of the disassembly
	//void SearchForFuncSigs(BOOL bNewME711);//Looks for signatures of commonly known functions and set their name.
	void SearchForDTCFlagSetting();//Looks for Bosch DTC setting fields.
	void SearchForFuncSigsAndThenCmt();//Looks for specific binary patterns and then makes a subroutine and comments it
	void SearchForArrayOffsetsAndThenCreate();//Looks for instructions that will probably contain an offset. When found it creates them.
	void loadASAPfile();// Loads ASAP file, and applies debug data to current database.  
};
