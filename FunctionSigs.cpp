...
//////////////////////////////////////////////////////////////////////
#define NUM_FUNC_SIGS	106 // update with number of sigs
...
unsigned char fs_functionnames[NUM_FUNC_SIGS][255] = 
		{
			...
			{"Fn_Maplookup2"},
			{"Fn_Maplookup3"},
			{"Fn_Maplookup4"},//Wlookup1D_RPM
			{"Fn_Maplookup5"},//BLookup2d_Fuel
			{"ParseDTCME71a"},
			{"SetDTCME71a"},
			{ "CopyBuffer"},
			{ "CRCSub0"},
			...
			{ "Fn_Maplookup6" }, //LookupA 
			{ "Fn_Maplookup7" }, // LookupB
			{ "Fn_Maplookup8" }, // LookupC
			{ "Fn_Maplookup9" },// LookupD
			{ "Fn_Maplookup10" },// LookupE
			{ "Fn_Maplookup11" },// LookupF
			{ "Fn_Maplookup12" },// LookupG
			{ "Fn_Maplookup13" },// LookupH
			{ "Fn_Maplookup14" },// LookupI
			{ "Fn_Maplookup15" },// LookupJ
			{ "Fn_Maplookup16" },// LookupK
			{ "Fn_Maplookup17" },// LookupL
			{ "Fn_Maplookup18" },// LookupM
			{ "Fn_Maplookup19" },// LookupN
			{ "Fn_Maplookup20" },// LookupO
			{ "Fn_Maplookup21" },// LookupP
			{ "Fn_Maplookup22" },// LookupQ
			{ "Fn_Maplookup23" },// LookupR
			{ "Fn_Maplookup24" },// LookupS
			{ "Fn_Maplookup25" },// LookupT
			{ "Fn_Maplookup26" },// LookupU
			{ "Fn_Maplookup27" },// LookupV
			{ "Fn_Maplookup28" },//WLookup1DIAT
			{ "Fn_Maplookup29" },// LookupX
			{ "Fn_Maplookup30" },// LookupY
			{ "Fn_Maplookup31" },// LookupZ
			{ "Fn_Maplookup32" },// LookupAA
			{ "Fn_Maplookup33" },//WLookup2DBoost
			{ "Fn_Maplookup34" },// LookupAC
			{ "Fn_Maplookup35" },// LookupAD
			{ "Fn_Maplookup36" },// LookupAE
			{ "Fn_Maplookup37" },// LookupAF
			{ "CRC8Sub" },
			{ "CRC8ME75" },
			{"RSACallingSub"},
			{"CRC32ME71Sub"},
...
unsigned char fs_functioncomments[NUM_FUNC_SIGS][255] = 
		{
			...
			{"Multipoint Checksum Routine"},
			{"Copies ROM Code to RAM Area\nProbably for FLASH programming"},
			{"Map lookup 1D 8 bytes\n\nnormally scans SPSNV08_UC, TMOV08_UC, RLV08_UC, or NMV08_UC\nr12= Map address, 1st byte height\nr13= X\nindirect addressing without DPP - just put 0x81 in front of R12"},
			{"map"},
			...
			{"Lookup using r4=axis1, r12=offset, r13= dpp. loaded by a helper routine.  Looks up KFTZWOB, KFWEETM, KFDLBTS, KFDTMTU, KFLDIWL, KFTEKA, KFDZWKG etc "},
			{"Looks up FLRFKATE via a helper function"},
			{"Looks up KFLDIAPL, KFETATE, "},
			...
			{"Main ROM chskum during runtime"},
			{"Combo lookup Fns 8 and 9 and 32"},
			{"CAN routine, contains B_statmdc"}
		};
...
//			 
			...
			//Fn_CombineCandDandAA
			{0x66, 0xFF, 0xFF, 0x00, 0x88, 0xF0, 0xF0, 0x4C, 0xF0, 0xCD, 0xF0, 0xDE, 0xC0, 0x8E, 0xD4, 0xF0, 
			0x0A, 0x00, 0xDA, 0x82, 0xFF, 0xFF, 0xF0, 0xA4, 0xD4, 0xC0, 0x02, 0x00, 0xD4, 0xD0, 0x04, 0x00, 
			0xA9, 0x80, 0xC0, 0x8E, 0xD4, 0xF0, 0x0C, 0x00, 0xDA, 0x82, 0xFF, 0xFF, 0x88, 0x40, 0x88, 0xA0, 
			0xD4, 0xC0, 0x0A, 0x00, 0xD4, 0xD0, 0x0C, 0x00, 0xE0, 0x4E, 0x00, 0xE0, 0x66, 0xFE, 0xFF, 0x3F, 
			0xE6, 0xFF, 0x03, 0x00, 0xDA, 0x82, 0xFF, 0xFF, 0x08, 0x06, 0xDB, 0x00, 0xDB, 0x00},
			//CAN
		//	{0x88, 0x90, 0x88, 0x80, 0x88, 0x70, 0x88, 0x60, 0x26, 0xF0, 0xFF, 0x00, 0x9A, 0xFF, 0xFF, 0xFF,
		//	0xC2, 0xF4, 0xFF, 0xFF, 0x46, 0xF4, 0x99, 0x00, 0xBD, 0x18, 0xF3, 0xF8, 0xFF, 0xFF, 0x2D, 0x04,
		//	0x29, 0x81, 0xF7, 0xF8, 0xFF, 0xFF, 0x0D, 0x16, 0x8A, 0x5A, 0xFF, 0xFF, 0xF2, 0xF4, 0xFF, 0xFF,
		//	0x8A, 0xF4}
			...
			0x88, 0x70,                             	//mov     [-r0], r7
			0x88, 0x60,                             //mov     [-r0], r6
			0x26, 0xF0, 0xFF, 0xFF,                 //sub     r0, #XXh
			0x9A, 0xFF, 0xFF, 0xFF,                 //jnb     XX, YY
			0xC2, 0xF4, 0xFF, 0xFF,                 //movbz   r4, byte_380A50
			0x46, 0xF4, 0x99, 0x00,                 //cmp     r4, #99h ; 'Ö'
			0xBD, 0xFF,                             //jmpr    cc_SLE, XX
			0xF3, 0xF8, 0xFF, 0xFF,                 //movb    rl4, XX
			0x2D, 0xFF,                             //jmpr    cc_Z, XX
			0x29, 0x81,                             //subb    rl4, #1
			0xF7, 0xF8, 0xFF, 0xFF,                 //movb    XX, rl4
			0x0D, 0xFF,                             //jmpr    cc_UC, loc_83133C
			0x8A, 0xFF, 0xFF, 0xFF,                 //jb      XX, XX
			0xF2, 0xF4, 0xFF, 0xFF,                 //mov     r4, XX
			0x8A, 0xF4}				//jb
		};
...
unsigned int fs_siglen[NUM_FUNC_SIGS] = {6, 62, 104, 98, 58, 252, 108, 202, 64, 40, 28, 44, 58, 120, 150, 66, 96, 
								...
								20, 27, 76, 64, 150, 64, 58, 58, 62, 78, 50};
...
unsigned char ts_Tablesigs[NUM_DATA_SIGS][310] = 
		{
			...
			// CWSAWE
			{0xC2, 0xF4, 0xFF, 0xFF, 0x68, 0x41, 0x2D, 0x0D, 0xF2, 0xF4, 0xFF, 0xFF, 0x42, 0xF4, 0xFF, 0xFF,
			0x8D, 0x03, 0xF2, 0xF4, 0xFF, 0xFF, 0x0D, 0x02, 0xF2, 0xF4, 0xFF, 0xFF, 0xF6, 0xF4}

		};

unsigned int ts_siglen[NUM_DATA_SIGS] = {128, 256, 36, 88, 36, 64, 60, 60, 64, 38, 38, 16, 16, 16, 40, 40, 20, 20, 16, 30};
...
ea_t FunctionSigs::CreateFunctionAndComment(ea_t eaAddr, unsigned char* pFuncName, unsigned char* pComment, unsigned int iTries)
{
	...
	if(add_func(eaAddr,BADADDR))
	{
		...
		qsnprintf(ucBuf, 40, "%s_%x", pFuncName, eaAddr);//Create a unique name with the address on the end
	//	qsnprintf(ucBuf, 40, "%s", pFuncName, eaAddr);//Create a unique name with no stuff appended
